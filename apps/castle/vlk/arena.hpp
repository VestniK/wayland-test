...
enum class image_purpose { texture = std::to_underlying(buffer_purpose::ibo) + 1 };

template <typename E>
concept memory_purpose = std::same_as<E, buffer_purpose> || std::same_as<E, image_purpose>;
...
struct purpose_traits<buffer_purpose> {
  ...
  static constexpr buffer_purpose values[] = {buffer_purpose::vbo, buffer_purpose::ibo};
};
...
template <memory_purpose Purpose>
inline constexpr size_t purposes_count = purpose_traits<Purpose>::purposes_count;

template <memory_purpose Purpose>
inline constexpr auto& all_purposes = purpose_traits<Purpose>::values;

constexpr size_t purpose_idx(memory_purpose auto purpose) noexcept { return static_cast<size_t>(purpose); }

namespace detail {

inline constexpr size_t purpose_data_arity = purposes_count<buffer_purpose> + purposes_count<image_purpose>;

template <typename V>
struct purpose_data : std::array<V, purpose_data_arity> {
  V& operator[](memory_purpose auto idx) noexcept { return (*this)[purpose_idx(idx)]; }
  const V& operator[](memory_purpose auto idx) const noexcept { return (*this)[purpose_idx(idx)]; }

  using std::array<V, purpose_data_arity>::operator[];
};

template <typename F>
concept query_memreq_function = requires(F&& f, buffer_purpose bp, image_purpose ip) {
...
concept device_allocation_function = requires(F&& f, uint32_t mem_type_bits, size_t sz) {
...
public:
  ...
  template <memory_purpose Purpose>
  std::tuple<Memory&, memory_region> lock_memory_for(Purpose p, size_t sz, size_t alignment);
