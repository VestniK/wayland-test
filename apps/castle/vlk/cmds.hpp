...
public:
  ...
  command_buffers(command_buffers&& rhs) noexcept
      : vk::raii::CommandPool{static_cast<vk::raii::CommandPool&&>(rhs)}, queue_{std::move(rhs.queue_)},
        bufs_{rhs.bufs_} {}
  command_buffers& operator=(command_buffers&& rhs) noexcept {
    clear();
    static_cast<vk::raii::CommandPool&>(*this) = static_cast<vk::raii::CommandPool&&>(rhs);
