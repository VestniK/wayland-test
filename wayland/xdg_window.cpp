...
toplevel_window::toplevel_window(
    wl_compositor& compositor, xdg_wm_base& shell, delegate* delegate)
...
void toplevel_window::set_delegate(delegate* delegate) noexcept {
  wl_proxy_set_user_data(
      reinterpret_cast<wl_proxy*>(xdg_surface_.get()), delegate);
  wl_proxy_set_user_data(
      reinterpret_cast<wl_proxy*>(toplevel_.get()), delegate);
}
...
void toplevel_window::configure_surface(
...
void toplevel_window::configure_bounds(void* data [[maybe_unused]],
    xdg_toplevel* xdg_toplevel [[maybe_unused]], int32_t width [[maybe_unused]],
    int32_t height [[maybe_unused]]) {}
void toplevel_window::wm_capabilities(void* data [[maybe_unused]],
    xdg_toplevel* xdg_toplevel [[maybe_unused]],
