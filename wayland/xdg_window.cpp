...
toplevel_window::toplevel_window(wl_compositor &compositor, xdg_wm_base &shell,
    ...
    : surface_{wl::unique_ptr<wl_surface>{
      ...
      xdg_surface_{wl::unique_ptr<xdg_surface>{
          xdg_wm_base_get_xdg_surface(&shell, surface_.get())}},
      toplevel_{wl::unique_ptr<xdg_toplevel>{
          xdg_surface_get_toplevel(xdg_surface_.get())}} {
...
void toplevel_window::set_delegate(delegate *delegate) noexcept {
  wl_proxy_set_user_data(reinterpret_cast<wl_proxy *>(xdg_surface_.get()),
                         delegate);
  wl_proxy_set_user_data(reinterpret_cast<wl_proxy *>(toplevel_.get()),
                         delegate);
}
...
void toplevel_window::configure_surface(void *, xdg_surface *surf,
                                        uint32_t serial) {
  xdg_surface_ack_configure(surf, serial);
}

void toplevel_window::configure_toplevel(void *data, xdg_toplevel *,
...
void toplevel_window::configure_bounds(void *data [[maybe_unused]],
                                       xdg_toplevel *xdg_toplevel
                                       [[maybe_unused]],
                                       int32_t width [[maybe_unused]],
                                       int32_t height [[maybe_unused]]) {}
void toplevel_window::wm_capabilities(void *data [[maybe_unused]],
                                      xdg_toplevel *xdg_toplevel
                                      [[maybe_unused]],
...
void toplevel_window::leave_output(void *data, wl_surface *,
                                   wl_output *output) {
  auto *self = reinterpret_cast<delegate *>(data);
  self->display(*output);
}
