...
// chaiscript конвертирует исключения между плюсами и скриптом, позволяя в скрипте ловить любые плюсовые исключения
// отнаследованные от std::exception. Исключение для принудительной остановки скрипта нельзя довать перехватывать
...
void add_unit_class(chaiscript::Module& module, const std::string& name) {
  chaiscript::utility::add_class<Unit>(module, name,
      {chaiscript::constructor<Unit()>(), chaiscript::constructor<Unit(const Unit&)>()},
      {
          {chaiscript::fun([](Unit l, Unit r) { return l + r; }), "+"},
          {chaiscript::fun([](Unit l, Unit r) { return l - r; }), "-"},
          {chaiscript::fun([](typename Unit::rep num, Unit unit) { return num * unit; }), "*"},
          {chaiscript::fun([](Unit unit, typename Unit::rep num) { return unit * num; }), "*"},
          {chaiscript::fun([](Unit unit, typename Unit::rep num) { return unit / num; }), "/"},
          {chaiscript::fun([](Unit l, Unit r) { return l / r; }), "/"},
      });
  if constexpr (std::is_integral_v<typename Unit::rep>) {
    module.add(chaiscript::fun([](Unit unit, typename Unit::rep num) { return unit % num; }), "%");
...
chaiscript::ModulePtr create_chrono_module() {
  ...
  res->add_global_const(chaiscript::const_var(std::chrono::duration_cast<std::chrono::milliseconds>(1s)), "s");
  res->add_global_const(chaiscript::const_var(std::chrono::duration_cast<std::chrono::milliseconds>(1min)), "min");
  res->add_global_const(chaiscript::const_var(std::chrono::duration_cast<std::chrono::milliseconds>(1h)), "h");
  return res;
}
...
chaiscript::ModulePtr create_wnd_module() {
  ...
  res->add(chaiscript::fun([](script_window& wnd) {
    if (auto ec = wnd.draw_frame()) {
      ...
      "draw_frame");
  res->add(chaiscript::fun([](script_window& wnd, std::chrono::milliseconds duration) {
