...
renderer::~renderer() { glDeleteProgram(program_); }
...
void renderer::draw(clock::time_point ts) {
  ...
  const GLfloat spot_phase =
      (ts.time_since_epoch() % spot_period).count() / GLfloat(clock::duration{spot_period}.count());

  const GLfloat spot_angle = 2 * M_PI * spot_phase;
  const GLfloat angle = 2 * M_PI * phase;
  glm::vec4 hotspot = {
      len_ / 2 + 7 * len_ / 24 * std::cos(spot_angle), 5 * len_ / 12 + len_ / 24 * std::cos(2 * spot_angle), 0, 0};
