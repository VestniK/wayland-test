...
gui_shell::create_maximized_window(event_loop &eloop,
  ...
  shell_window wnd;
  if (get_ivi())
    wnd = shell_window{ivi::window{*get_compositor(), *get_ivi(),
                                   ivi_main_glapp_id, &szdelegate}};
  else {
  ...
  co_return sized_window<shell_window>{.window = std::move(wnd),
                                       .sz = szdelegate.wnd_size.value()};
}

void gui_shell::global(void *data, wl_registry *reg, uint32_t id,
