...
public:
  ...
  constexpr explicit attrib_location(GLint location) noexcept : location_{location} {}
...
void attrib_location<glm::vec3>::set_pointer(member_ptr<C, glm::vec3> member) {
  glVertexAttribPointer(
      location_, 3, GL_FLOAT, GL_FALSE, sizeof(C), reinterpret_cast<const GLvoid*>(member_offset(member))
  );
...
void attrib_location<glm::vec2>::set_pointer(member_ptr<C, glm::vec2> member) {
  glVertexAttribPointer(
      location_, 2, GL_FLOAT, GL_FALSE, sizeof(C), reinterpret_cast<const GLvoid*>(member_offset(member))
  );
  glEnableVertexAttribArray(location_);
}
