...
constexpr unsigned vertixies_in_hexagon = vertixies_in_triangle * triangles_in_hexagon;

template <size_t N>
std::array<glm::vec2, vertixies_in_triangle * N>
get_triangles(const hexagon_tiles<glm::vec2>& land, size_t start_offset) noexcept {
  std::array<glm::vec2, vertixies_in_triangle * N> triangles_set;
  const auto indexes =
      std::span<const unsigned>{land.indexes()}.subspan(start_offset, vertixies_in_triangle * N);
  std::transform(
  ...
  SECTION("covers flat rectangular area") {
    ...
    SECTION("mesh has proper x dimensions") {
      CHECK(
          ...
          Catch::Approx(
              (2 + (1 + std::cos(M_PI / 3)) * (columns - 1)) * radius.numerical_value_in(tiles_unit)
          )
      );
    }
    SECTION("mesh has proper y dimensions") {
      CHECK(
          landscape_bounds.width() ==
          Catch::Approx(rows * 2 * radius.numerical_value_in(tiles_unit) * std::sin(M_PI / 3))
      );
    }
  }
      ...
      SECTION(fmt::format(
        ...
        for (size_t triangle_offset = 0; triangle_offset < triangles_set.size();
          ...
          SECTION(fmt::format("triangle {} is equilateral", triangle_offset / triangles_in_hexagon)) {
            ...
            CHECK(
                glm::dot(triangle[1] - triangle[0], triangle[2] - triangle[0]) ==
                Catch::Approx(equilateral_triangle_sides_dot_prod)
            );
            CHECK(
                glm::dot(triangle[0] - triangle[1], triangle[2] - triangle[1]) ==
                Catch::Approx(equilateral_triangle_sides_dot_prod)
            );
            CHECK(
                glm::dot(triangle[0] - triangle[2], triangle[1] - triangle[2]) ==
                Catch::Approx(equilateral_triangle_sides_dot_prod)
            );
          }

          SECTION("there are 7 unique points (center and hexagon perimeter)") {
            std::sort(triangles_set.begin(), triangles_set.end(), [](glm::vec2 l, glm::vec2 r) {
