...
TEST_CASE("triangular to cartesian", "[triangular]") {
  ...
  SECTION("x coordinate calculated properly") {
    CHECK_THAT(
        triangular::to_cartesian({tx, ty}).x, Catch::Matchers::WithinAbs(tx + ty * std::cos(M_PI / 3), 0.0001)
    );
  }

  SECTION("y coordinate calculated properly") {
    CHECK_THAT(
        triangular::to_cartesian({tx, ty}).y, Catch::Matchers::WithinAbs(ty * std::sin(M_PI / 3), 0.0001)
    );
  }
}

TEST_CASE("hexagon_net::cell_center returns zero point for 0,0 cell", "[hexagon_net]") {
...
TEST_CASE("hexagon_net::cell_center", "[hexagon_net]") {
  ...
  CAPTURE(m, n, cell_center(m, n), cell_center(m, n + 1), cell_center(m + 1, n));

  SECTION("adjacent cells from the same column have common upper border") {
    CHECK(
        cell_coord(cell_center(m, n), corner::top_left) ==
        cell_coord(cell_center(m, n + 1), corner::bottom_left)
    );
    CHECK(
        cell_coord(cell_center(m, n), corner::top_right) ==
  ...
  SECTION("adjacent cells from the same row have proper common border") {
    CHECK(
        cell_coord(cell_center(m, n), m % 2 == 0 ? corner::top_right : corner::bottom_right) ==
        cell_coord(cell_center(m + 1, n), corner::midle_left)
    );
    CHECK(
        cell_coord(cell_center(m, n), corner::midle_right) ==
        cell_coord(cell_center(m + 1, n), m % 2 == 0 ? corner::bottom_left : corner::top_left)
    );
  }
}
