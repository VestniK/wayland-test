...
std::unique_ptr<executors_environment::environment_data> executors_environment::environment_data_instance;
...
void executors_environment::testRunEnded(const Catch::TestRunStats&) { environment_data_instance.reset(); }
...
void executors_environment::wait_pool_tasks_done() {
  for (size_t i = 0; i < threads_count; ++i)
    asio::post(environment_data_instance->pool, [] { environment_data_instance->barrier.arrive_and_wait(); });
