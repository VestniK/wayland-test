...
load_absinfo(asio::posix::stream_descriptor& dev, gamepad::axis axis, gamepad::dimention coord) {
  return evio::axis2evcode(axis, coord).transform([&](auto axe) { return load_absinfo(dev, axe); });
}
...
std::string load_dev_name(asio::posix::stream_descriptor& dev) {
  std::string name;
  name.resize(256);
  if (ioctl(dev.native_handle(), EVIOCGNAME(name.size() - 1), name.data()) == -1)
  ...
  while (dev.is_open()) {
    ...
    while (bytes_read == 0 || bytes_read % sizeof(input_event) != 0) {
      ...
      bytes_read += co_await dev.async_read_some(
          span2buf(std::as_writable_bytes(std::span{events}).subspan(bytes_read)), asio::deferred
      );
    }
