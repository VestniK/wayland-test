...
void log_gamepad_info(
    std::string_view action, std::string_view devnode, udev_device& dev) {
  const char* sysnum = udev_device_get_sysnum(&dev);
  const char* syspath = udev_device_get_syspath(&dev);

  using detail::udev::list_range;
  spdlog::debug("{} gamepad:\n\t{}\n\t{}\n  sysnum: {}\n  "
                "devpath: {}\n  subsystem: {}\n  devtype: {}\n  props:\n\t{}",
...
udev_gamepads::connected_gamepads::connected_gamepads(udev& udev)
  ...
  udev_enumerate_add_match_property(
      enumerator_.get(), "ID_INPUT_JOYSTICK", "1");
  if (int ret = udev_enumerate_scan_devices(enumerator_.get()); ret < 0)
    throw std::system_error{
        -ret, std::system_category(), "udev_enumerate_scan_devices"};
}

asio::awaitable<void> udev_gamepads::watch(
